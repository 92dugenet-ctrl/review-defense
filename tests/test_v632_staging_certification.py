...
def test_live_metrics_are_not_public_contract(monkeypatch):
    ...
    def fake_fetch(base, path):
        calls.append(path)
        if path == "/health": return 200, {"version": "6.36", "service": "review-defense"}, {"content-security-policy":"x", "x-content-type-options":"nosniff", "x-frame-options":"DENY", "referrer-policy":"no-referrer"}
